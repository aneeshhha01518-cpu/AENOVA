...
function App() {
  ...
  const saveProfile = async () => {
    ...
    if (!profile.college.trim()) {
      setProfileStep(2);
      setProfileMessage("Please enter your college name.");
      return;
    }

    if (!profile.department.trim()) {
      setProfileStep(2);
      setProfileMessage("Please enter your department.");
      return;
    }

    if (!profile.study_year.trim()) {
      setProfileStep(2);
      setProfileMessage("Please enter your study year.");
      return;
    }

    if (!profile.location.trim()) {
      setProfileStep(3);
      setProfileMessage("Please enter your city or location.");
      return;
    }

    if (!profile.skills.trim()) {
      setProfileStep(4);
      setProfileMessage("Please add at least one skill.");
      return;
    }

    if (!profile.interests.trim()) {
      setProfileStep(4);
      setProfileMessage("Please add your interests.");
      return;
    }

    if (!profile.career_goal.trim()) {
      setProfileStep(5);
      setProfileMessage("Please enter your career goal.");
      return;
    }
    ...
    try {
      ...
      if (!returnedProfileId) {
        throw new Error(
          "Profile was saved, but the backend did not return its ID."
        );
      }
      ...
      if (!Number.isFinite(numericProfileId)) {
        throw new Error(
          "Profile was saved, but the returned profile ID is invalid."
        );
      }
      ...
      // IMPORTANT: tell the student immediately.
      setProfileMessage(
      ...
      // Guide the student immediately instead of making them wait here.
      scrollToSection("recommendations");

      // This call only reads stored opportunities and scores them.
      // Live opportunity collection is not part of this request.
      ...
      if (ready) {
        ...
        setProfileMessage(
          "✓ Profile saved successfully! There are no suitable matches in the currently stored listings."
        );
      }
